...
### Added
The following contracts have been deployed across all major test networks:
...
Furthermore, the following changes have been implemented:
- Added `1_initial_migration.js` file to the migrations folder;
- Refactored the `truffle-config.js` file with a general dependency on the newly added file `network-config`, where all major test networks are defined as well as the Swiss DLT chain;
